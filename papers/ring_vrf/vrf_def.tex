...
\begin{itemize}
 \item \adv receives $\pk$ from \ora{Sign} of Definition \ref{def:vrf_sign_oracle},
  repeatedly queries \ora{Sign}, and produces $\Out_0,\compk,\openpk,\msg,\aux$.
 \item If \adv never queried $\ora{Sign}(\cdot,\cdot,\msg,\cdot)$ then
...
EC VRF signatures have the form $\sigma = (\Out_0,\pi)$ in which some
inner proof $\pi$ proves correctness of some associated VUF output $\Out_0$. % aka ``pre-output''.  % ``pre-pseudo-random''
It follows $\VRF.\Eval$ never corresponds to $\Out_0$, but if one describes
protocols with an \primalgo{EvalProve} formulation then exposing $\Out_0$
...
% We need $\Out_0$ to be exposed by implementations so ...
