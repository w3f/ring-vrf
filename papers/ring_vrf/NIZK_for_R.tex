...
\noindent \textbf{Special Perfect Completeness} For all $\lambda \in \mathbb{N}$, for every efficient adversary $A$, \qhandan{for every 
 $(\barz, \barx; \baromega_2) \in \reltwo$ it holds}{It is a completeness definition for $ \reltwo $ but there is nothing related to $ \reltwo $ below. We want to define a new completeness definition for $ \nizkR $. So, I suggest you to replace everything before given $ (|) $ with $ \nizkR.\Verify((\crsR,(\bary, \barz), (\pi_1, \pi_2, X)))\rightarrow 1 $ because everything you have before $ | $are already satisfied from the $ \NIZK $ completeness and $ \ZKCont $ special completeness, so they are already satisfied, they are not new statements. }:
...
\noindent \textbf{Zero-knowledge \qhandan{after Reusing a $\ZKCont$ Proof}{I think this naming is misleading because we don't reuse zk cont proof instead we run Reprove on it.}} For all $\lambda \in \mathbb{N}$, for every benign auxiliary input $aux$, 
for all $\bary,\barx,\barz, \baromega_1, \baromega_2$ with $(\bary, \barx; \baromega_1) \in \relone$ and $(\barz, \barx; \baromega_2) \in \reltwo$, \handan{for all $X',\pi_1',\pi_2, b'$}{for all $X',\pi_1', b'$}, for every adversary $A$ it holds:
\begin{align*}
...
\begin{lemma}[ZK after Reusing a $\ZKCont$ Proof] 
