...
Let $\ZKCont$ be a zk continuation for $\relone$ (from preamble of Section~\ref{sec:rvrf_cont}) and let $\nizktwo$  be a NIZK 
...
\begin{align*}
\mathit{Pr}(& A(\crs, \realaux, \pi'_1,\pi_2, X', \rel) = 1 \ | \ (\crs, \pp) \leftarrow \ZKCont.\Setup (1^{\lambda}), \\
...
= \mathit{Pr}(& A(\crs, \realaux, \pi',\pi_2, X', \rel) = 1 \ | \ (\crs, \pp) \leftarrow \ZKCont.\Setup (1^{\lambda}), \\ 
