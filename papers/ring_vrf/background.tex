...
\begin{definition}\label{def:nizk_completeness}
	...
	$ \secparam $, for all $ (\rel,z) \leftarrow \mathfrak{R} $, for all $(x; \omega) \in \rel$ where $ \pi \leftarrow \NIZK_\rel.\Prove(\crs_\rel, \pp_\rel, x; \omega) $.  % with high probability
\end{definition}
...
\begin{definition}[Non-interactive knowledge of arguments (NARK)] $ \NARK_\rel $ for a relation $ \rel $ consists of the same algorithms $ \NIZK $ but satisfies only completeness (Definition \ref{def:nizk_completeness}) and knowledge soundness (Definition \ref{def:nizk_knowledge_sound}).
\end{definition}
