...
Ring VRFs differ from VRFs in that they do not expose a signer,
and instead prove the signer's key lies in  \ring,
\eprint{much like how ring signatures differ from signatures}{ like ring signatures}.
...
After \eprint{successful}{the} verification, our verifier should be convinced that $\pk \in \ring$, that
...
as otherwise the ring VRF signature become unmoored and permits replay attacks \eprint{}{(See \S\ref{sec:app_short}).}
\eprint{As an example, our identity protocol below in \S\eprint{\ref{sec:app_identity}}{\ref{sec:app_short}}
