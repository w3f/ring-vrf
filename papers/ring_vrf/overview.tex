...
We demand pseudo-randomness properties from \Eval, which either exactly
mirror to \cite{vrf_micali} or closely resemble \cite{praos,ucvrf}.

Ring VRFs differ from VRF in that they do not expose a specific signer,
...
After success, our verifier should be convinced that $\pk \in \ring$ and
...
signer, except that their key maps $\msg \mapsto \Out$.
Importantly, pseudo-randomness also says that \Out {\it is} an identity
...
with some set commitment to \ring, roughly like what ZCash does.
\begin{itemize}
