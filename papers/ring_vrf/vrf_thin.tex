\subsection{Thin batchable 2C 1sMSM EC VRF-AD}
\label{sec:vrf_thin}
...
As a reslt, their naively batchable variant become ``fat'', requiring
one scalar and two seperate nonces, which complicates batch verification.

We propose a new ``thin'' batchable EC VRF that merges these two nonces,
and requires only one elliptic curve multi-exponentiation (1sMSM), but
internally computes an extra delinearization challenge (2C).
As such, our thin batchable variant both runs faster and simplifies heafy usage,
and also provides a less annoying interface.

Interestingly, our thin batchable EC VRF winds up litterally being
...
because our ``fat Pederson'' variant introduced next in \S\ref{sec:vrf_pederson}
...
meaing $\ThinVRF.\CommitKey$ and $\ThinVRF.\OpenKey$ are trivial, and $\openpk = \emptyset$.

We work solely in $\ecE$ here because we need only a basic Chaum-Pederson DLEQ proof.
...
%PoK:    \item abort unless either $\msg$ contains $\pk$ or else \pk has a valid the proof-of-knowledge,
    \item recompute the VRF input point $\In := H_{\grE}(\msg,\pk)$,
    \item recompute $c_1 = H_p(\aux,\msg,\pk,\Out_0)$ and $c = H_p(\aux,\msg,\pk,\Out_0,R)$, % the challenges
    \item abort unless $s h (\genE + c_1 \In) = h R + c h (\pk + c_1 \Out_0)$ holds, and 
    \item return $H'(\msg,\pk,h \Out_0)$ if nothing failed.
\end{enumerate}
\end{itemize}
...
If desired, one could easily generalize this VRF to $k$ messages by computing $k$ distinct delinearization challenges $c_j = H_p(\aux,\msg_1,\ldots,\msg_k,\pk,\Out_{0,1},\ldots,\Out_{0,k},j)$ for $j=1,\ldots,k$.
...
We discuss chosen message queries against only one key in pseudorandomness.  
% TODO: What?
...
wehn combining identical inputs \msg with related secret keys,
...
our $\ThinVRF$ satisfies
 VRF correctness, uniqueness, pseudorandomness,
 and existential unforgability on $(\msg,\aux)$.
\end{proposition}
...
As $H'$ is a PRF, we now argue pseudorandomness for$\ThinVRF$ similarly
