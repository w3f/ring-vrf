...
$\cQ$ we define $\relRQ$ over pairs $((\bary, \barx); \baromega) \in \F_q^{l} \times \F_q^{n-l} \times \F_q^{m-n}$ that holds iff there 
...
$\crspk = {\color{red}\ldots}$ and $\crsvk = {\color{red}\ldots}$.  
...
&C = \sum_{i=n+1}^{m} v_i \frac{\beta a_i(\tau)+ \alpha b_i(\tau)+ c_i(\tau)}{\delta}   + As + Br - rs\delta, 
\end{align*}
...
where $\barx = (x_{l+1}, \ldots, x_n)$, $ 0 \leq l \leq n$. 
...
where $Y = \sum_{i=1}^{l} x_i [\frac{\beta a_i(\tau)+ \alpha b_i(\tau)+ c_i(\tau)}{\gamma}]_1$ and $\bary = (x_1, \ldots, x_l)$.

\item $\ZKCont.\Sim: (\tw, \bary, \relRQ) \mapsto \pi$ where \\
\end{itemize} 
\end{definition}
