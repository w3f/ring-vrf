...
\mathit{Pr}(&(\crs, \pp) \leftarrow \ZKCont.\Setup (1^{\lambda}), (\crspk, \crsvk) \leftarrow \ZKCont.\Gen(\crs, \relone), \\ 
                  & (\pi', X', b') \leftarrow \ZKCont.\Reprove (\crspk, X, \pi, b, \relone) \ | \\ 
...
Below we instantiate our zero-knowledge continuation notion starting from the Groth16~\cite{Groth16} SNARK;
hence, we call our instantiation \emph{specialised Groth16} or \emph{special G}. 

\begin{definition}[Specialised Groth16]
\label{insta:sg16} 
We call specialised Groth16 the following instantiation of our zero-knowledge continuation notion in Definition~\ref{def:zk_cont}:
\begin{itemize}
\item $\ZKCont.\Setup: (1^{\lambda}) \mapsto (\crs, \pp, \tw)$. \\ 
\noindent Pick $\alpha, \beta, \gamma, \delta, \tau  \xleftarrow{\$} \F_q^{*}$.  
Set $\tw = (\alpha, \beta, \gamma, \delta, \tau, ??)$. Set $\crs = ()$ 
\item
\item
\item
\item
