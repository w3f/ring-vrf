...
%\begin{definition}[ZK Continuations] A zero-knowledge continuation $\specg_\rel$ consists of four algorithms 
%($\specg_\rel.\Preprove$, $\specg_\rel.\Reprove$, $\ldots$, $\specg_\rel.\Verify$) such that:
%\begin{itemize}
%\item $\specg_\rel.\Preprove : (\bar{y}, \bar{x}; \bar{\omega}) \mapsto (X,\pi)$ \,
...
%\item $\specg_\rel.\Reprove : (X,\pi) \mapsto ((X',\pi'); b)$ \,
%finalises the commitment $X'$ and proof $\pi'$ and returns an opening $b$ for the commitment. 
%\item $\specg_\rel.\Verify(\bar{y}; (X',\pi') )$ \, 
%verifies the 
%\end{itemize}
%%TO DO: add an algorithm to the $\specg_\rel$ such that: Our \Verify needs a separate proof-of-knowledge for $X'$, 
...
\noindent \textbf{Perfect Zero-knowledge w.r.t. $\relone$} For all $\lambda \in \mathbb{N}$, for all  $(\bary, \barx; \baromega) \in \relone$, 
for all $X$, for all $\pi$, for all $b$, for every adversary $A$ it holds:
\begin{align*}
\mathit{Pr}(& A(\crs, \pp, \pi', X', \relone) = 1 \ | \ (\crs, \pp) \leftarrow \ZKCont.\Setup (1^{\lambda}), \\
...
\begin{definition}[Specialised Groth16 ($\specg$)]
\label{insta:sg16} 
...
\begin{align*}
&b = 0; r, s\xleftarrow{\$} \F_p; X = \sum_{i=l+1}^{n} v_i [\frac{\beta a_i(\tau)+ \alpha b_i(\tau)+ c_i(\tau)}{\gamma}]_1; \pi = ([A]_1, [B]_2, [C]_1); \\
...
where $\pi' = (U', V', W')$, $Y = \sum_{i=1}^{l} x_i [\frac{\beta a_i(\tau)+ \alpha b_i(\tau)+ c_i(\tau)}{\gamma}]_1$ 
...
\noindent \paragraph{Notes:} Our $\specg$ instantiation (i.e., $\Reprove$ algorithm) uses a  
Groth16 re-randomisation technique, (see~\cite[Fig.~1]{RandomizationGroth16}). Moreover, the trusted setup required by 
$\mathsf{SpecialG}$ is the same as that required by original Groth16~\cite{Groth16} extended by two additional group 
elements $\Kgamma = [\frac{\eta}{\gamma}]_1$ and $\Kdelta = [\frac{\eta}{\delta}]_1$. An identical trusted setup as in 
our instantiation is used in~\cite[Fig.~22]{legosnark}. Note that, if a trusted setup is used (for example the one described 
in~\cite{subversion_zk}) such that there exist a public and efficient procedure for verifying it, then, by extending it with 
$\Kgamma$ and $\Kdelta$ (which is our extension of the standard Groth16 trusted setup), the resulting setup remains publicly 
verifiable (i.e., by using the additional verification equation $e(\Kgamma, [\gamma]_2) = e(\Kdelta, [\delta]_2)$), and, 
hence, according to~\cite{subversion_zk}, subversion zero-knowledge. 
...
input to $\specg.\Reprove$ is a possibly maliciously generated but otherwise a verifying proof. Hence, it is easy to see 
that the output $\pi'$ by the simulator $\specg.\Sim$ is identically distributed to a proof $\pi'$ output by 
$\specg.\Reprove$ so the perfect zero-knowledge property holds for $\specg$.
\end{proof}
...
We build {\em special Groth16} aka \specg by adding one fresh
...
our algorithm $\specg.\Reprove : (X,A,B,C) \mapsto ((X',A',B',C'); b)$ produces an
...
We thus write $\specg.\Preprove : (\bar{y}, \bar{x}; \bar{\omega}) \mapsto (X,A,B,C)$
...
At this point $\specg.\Verify(\bar{y}; (X',A',B',C') )$
...
transforms honestly generated \specg zero-knowledge continuation $(X,A,B,C)$
into identically distributed \specg proof $(X',A',B',C')$,
...
then our zero-knowledge continuation \specg satisfies knowledge soundness.
\end{lemma}
