...
\qhandan{finalises commitment $X$}{What does it mean?} and proof $\pi$ and returns an opening $b$ for the commitment, 
...
\item \qhandan{$\ZKCont.\Verify: (\crs, \bar{y}, X, \pi, \relone) \mapsto 0/1$}{I think we need some explanations or motivation about why we have the commitment of  $ \bar{x} $ instead of $ \bar{x} $ as an input of $ \Verify $} \, outputs $1$ in case it accepts \handan{}{the proof $ \pi $} and $0$ otherwise,
...
\end{align*}
\qhandan{}{We need a motivation here why we define completeness over valid proofs which are generated by an adversarial algorithms. Maybe we can mention some possible applications of ZKcont such as outsourcing PreProve}
...
for all  $(\bary, \barx; \baromega_1) \in \relone$, for all $X'$, for all $\pi'$, for all $b'$, for every adversary $A$ \handan{}{there exists a PPT algorithm $ \Sim $ such that}:
\begin{align*}
...
\end{comment}
 ...
 \qhandan{}{Maybe some remarks why we define ZK only for Reprove? Does it cover ZK for Preprove as well?}
\end{definition} 
...
\noindent \qhandan{Given notation provided in section~\ref{sec:background}, we introduce}{Could you check Section \ref{sec:background} and remove the unnecessary notations or explanations? Or could you make more clear which one of them are defined related to Groth16? I think the current version of this section looks complicated and it could be simplified}
...
Let $\crs = ([\barsig_1]_1, [\barsig_2]_2)$ where \qhandan{}{You can introduce $ []_1, []_2 $ in Section \ref{sec:background} if you want to use this notation and say that we use it only for Groth16 related things to be consistent with the original notation}
\begin{align*}
...
An identical trusted setup \handan{to}{} that used by \SpecialG was used in LegoSNARK~\cite[Fig.~22]{LegoSNARK} which defines 
...
AGM~\cite{Fuchs_AGM}, $\SpecialG$ is a zero-knowledge continuation as per \handan{definition}{Definition}~\ref{def:zk_cont}. 
\end{theorem}
...
\qhandan{commit-carrying SNARK with double binding (cc-SNARK with double binding) as per Definition 3.4~\cite{LegoSNARK}}{If it is easy, you should give intuition about what it is. What is the relation with the properties that we want to show for $ \SpecialG $?}. 
\qhandan{We use the fact that $\ccgroth$ as defined by the NILP detailed in Fig.22, Appendix H.5~\cite{LegoSNARK} satisfies that latter definition.}{I don't see in the proof you relate anything related to double binding property. Is this the fact you need? If yes, it should be more clear in the proof why you need it. From what I understood, you just use thex§x§x§ fact that ccSnark is KS.} Moreover, 
$\SpecialG$'s $\Setup$ together with \qhandan{$\Gen$}{is there any algorithm called $ \Gen $ for $ \SpecialG $?}, on one hand, and $\ccgroth$'s \handan{$\mathit{KeyGen}$}{$ \mathsf{KeyGen} $}, on the other hand, are the same procedure. Also $\SpecialG$ 
...
define adversary $A_{\ccgroth}$ for KS  in \qhandan{Definition 3.4~\cite{LegoSNARK}}{I guess it is 3.2}:
\begin{align*}
...
\noindent Given extractor $E_{\ccgroth}$ fulfilling \qhandan{Definition 3.4~\cite{LegoSNARK}}{I guess it is 3.2} for $A_{\ccgroth}$, we
...
\end{align*}
\qhandan{}{Related to the above statement, what does the output of $  \ E_{\SpecialG} $ has $ \bar{x} $ and $ b $ as in defined in Definition \ref{def:zk_cont}. From my understanding, $ \bar{x} $ and $ b $ in $ \ZKCont $ has the similar job as in $ \ccgroth $. }

We show $E_{\SpecialG}$ fulfils Definition~\ref{def:zk_cont} for $A_{\SpecialG}$. Assume by contradiction that is not the case. 
This implies there \handan{exist}{exists an} auxiliary input $\realaux$ such that each: 
\begin{align*}
\handan{\ZKCont}{\SpecialG}.\Verify(\crs, \bary, X, \pi, &\relRQ) =1 \ (10)\ ; \ \handan{\ZKCont}{\SpecialG}.\VerifyCom(\pp, X, \barx, b) =1 \ (20) \\
& (\bary, \barx; \baromega) \notin \relRQ  \ \ (30) 
\end{align*}
hold with non-negligible probability. Since $(20)$ holds with non-negligible probability and \qhandan{verification}{is it proof verification or commitment verification. It should be clear} is \qhandan{identical  for $\SpecialG$ and $\ccgroth$}{I don't think you mention this fact before. If it is the case, you should say it while describing $ \SpecialG $}, and since $E_{\ccgroth}$ is an extractor for $A_{\ccgroth}$ as per Definition 3.4~\cite{LegoSNARK},
 then each of the two events 
\begin{align*}
\mathit{VerCommit^*}(\pp, X, \barx^*, b^*) =1 \ (40) \ ; \ (\bary, \barx^*; \baromega^*) \in  \relRQ \ (50)
\end{align*}
\qhandan{}{Related to $ (40) $, I guess $ \mathit{VerCommit^*} $ is $ \ccgroth.\VerifyCom $. If it is the case, $ \ccgroth.\VerifyCom $ does not have an input called $ pp $}
...
\qhandan{}{I think a notation map would be very nice from $ \ccgroth $ and $ \SpecialG $ to verify all the claims above easily i.e., $ o \rightarrow b $, $ c \rightarrow X $...}
