...
\item $\ZKCont.\Sim: (\tw, \bary, \barx, \relone) \mapsto \pi$ takes as input a simulation trapdoor $\tw$ and statement $(\bary, \barx)$ and returns an argument $\pi$,
\end{itemize}
...
\mathit{Pr} (& (\ZKCont.\Verify(\crsvk, \bar{y}, X, \pi, \relone) = 1) \ \wedge \ (\ZKCont.\VerifyCom(X, \bar{x}, b) = 1) \ \wedge\ \\
                   & \wedge \ ( (\bary, \barx; \baromega) \notin \relone) \ | \ \crs \leftarrow \ZKCont.\Setup (1^{\lambda}), (\bary, \barx, X, \pi, b) \leftarrow A(\crs,\relone), \\ 
