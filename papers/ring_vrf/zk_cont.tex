...
($\ZKCont.\Setup$, $\ZKCont.\Gen$, $\ZKCont.\Preprove$, $\ZKCont.\VerifyCom$, $\ZKCont.\Reprove$, $\ZKCont.\Verify$) 
such that for implicit security parameter $\lambda$,
\begin{itemize}

\item $\ZKCont.\Setup: (1^{\lambda}) \mapsto (\crs, \pp)$ a setup algorithm that on input the security parameter 
...
\item $\ZKCont.\Reprove: (\crspk, X, \pi, {\color{red} b}, \relone) \mapsto (X', \pi', b')$ \,
...
\mathit{Pr} (& (\ZKCont.\Verify(\crsvk, \bar{y}, X, \pi, \relone) = 1  = >  \ZKCont.\Verify(\crsvk, \bar{y}, X', \pi', \relone) = 1)  \ \wedge \  \\
                   ...
                   & \crs \leftarrow \ZKCont.\Setup (1^{\lambda}), (\crspk, \crsvk) \leftarrow \ZKCont.\Gen(\crs, \relone), \\ 
                   & (\bary, \barx, X, \pi, b) \leftarrow A(\crs,\relone), (X', \pi', b') \leftarrow \ZKCont.\Reprove(\crspk, X, \pi, b, \relone)) = 1
\end{align*}
\noindent \textbf{Knowledge Soundness} For every efficient adversary $A$ there exists an efficient extractor $E$ such that 
\begin{align*}
\mathit{Pr} (& (\ZKCont.\Verify(\crsvk, \bar{y}, X, \pi, \relone) = 1 \ \wedge \ \ZKCont.\VerifyCom(X, \bar{x}, b) = 1) \ \wedge\ \\
                   & \wedge \ ( (\bary, \barx; \baromega) \in \relone) \ | \ \crs \leftarrow \ZKCont.\Setup (1^{\lambda}), (\bary, \barx, X, \pi, b) \leftarrow A(\crs,\relone), \\ 
& \baromega \leftarrow E^{A}(\crs, \relone)) = 1 - \negl (\lambda),
\end{align*}
