...
\mathit{Pr} (& (\ZKCont.\Verify(\crsvk, \bar{y}, X, \pi, \relone) = 1) \ \wedge \ (\ZKCont.\VerifyCom(\pp, X, \bar{x}, b) = 1) \ \wedge\ \\
                   & \wedge \ ( (\bary, \barx; \baromega_1) \notin \relone) \ | \ (\crs, \tw) \leftarrow \ZKCont.\Setup (1^{\lambda}), \\
...
\begin{align*}
\mathit{Pr}(& A(\crs, \realaux, \pi, X, \relone) = 1 \ | \ (\crs, \tw) \leftarrow \ZKCont.\Setup (1^{\lambda}), \\
                  & (\crspk, \crsvk, \pp) \leftarrow \ZKCont.\Gen(\crs, \relone), \\ 
...
\begin{align*}
&b' = 0; r, s\xleftarrow{\$} \F_p; X' = \sum_{i=l+1}^{n} v_i\left[ \frac{\beta a_i(\tau)+ \alpha b_i(\tau)+ c_i(\tau)}{\gamma} \right]_1; \pi' = ([o]_1, [u]_2, [v]_1); \\
...
$\SpecialG$'s $\Setup$ together with $\Gen$, on one hand, and $\ccgroth$'s $\mathit{KeyGen}$, on the other hand, are the same procedure. Also $\SpecialG$ 
...
\noindent Given extractor $E_{\ccgroth}$ fulfilling Definition 3.4~\cite{LegoSNARK} for $A_{\ccgroth}$, 
