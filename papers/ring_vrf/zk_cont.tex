...
\item \qhandan{$\ZKCont.\Verify: (\crs, \bar{y}, X, \pi, \relone) \mapsto 0/1$}{I think we need some explanations or motivation about why we have the commitment of  $ \bar{x} $ instead of $ \bar{x} $ as an input of $ \Verify $} \, outputs $1$ in case it accepts  and $0$ otherwise,
...
\qhandan{We use the fact that $\ccgroth$ as defined by the NILP detailed in Fig.22, Appendix H.5~\cite{LegoSNARK} satisfies that latter definition.}{I don't see in the proof you relate anything related to double binding property. Is this the fact you need? If yes, it should be more clear in the proof why you need it. From what I understood, you just use the  fact that ccSnark is KS.} Moreover, 
