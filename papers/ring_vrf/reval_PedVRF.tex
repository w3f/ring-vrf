\subsection{Putting NIZKs Together for $\rel$}
...
for all $\bary,\barx,\barz, \baromega_1, \baromega_2$ with $(\bary, \barx; \baromega_1) \in \relone \wedge (\barz, \barx; \baromega_2) \in \reltwo$, for all $X',\pi_1',\pi_2, b'$, for every adversary $A$ it holds:
\begin{align*}
| \mathit{Pr}(& A(\crs, aux, \pi_1,\pi_2, X, \rel \wedge \ZKCont.\Verify(\crsvk, \bary, X', \pi'_1, \relone) = 1 \\
                  & \wedge \VerifyCom(\pp, X', \barx', b') = 1) = 1 \ | \ (\crs, \pp) \leftarrow \ZKCont.\Setup (1^{\lambda}), \\
                  & (\crspk, \crsvk) \leftarrow \ZKCont.\Gen(\crs, \relone), (\pi_1, X, \_) \leftarrow \ZKCont.\Reprove (\crspk, X', \pi_1', b', \relone), \\
                  & \pi_2 \leftarrow \nizktwo.\Prove(\crspktwo, X, \barz, \barx, b, \baromega_2)) =  \\
- \mathit{Pr}(& A(\crs, aux, \pi_1,\pi_2, X, \rel) = 1 \wedge \ZKCont.\Verify(\crsvk, \bary, X', \pi'_1, \relone) = 1 \\
                    & \wedge \VerifyCom(\pp, X', \barx', b') = 1\ | \ (\crs, \pp) \leftarrow \ZKCont.\Setup (1^{\lambda}), \\ 
