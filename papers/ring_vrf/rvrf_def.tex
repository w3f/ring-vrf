...
We distrust group signatures thought because typically some group master
has deanonymization capabilities, giving them only narrow real world uses,
like mailbox spam defense in Pond \cite{pond}.

At first blush, attribute based credential schemes have typically
similarly empowering some group master, which maked them unsuitabe for
general purpose privacy.
At least one related scheme \cite{cryptoeprint:2021:181} makes efforts
towards giving users real control, albeit only in a byzantine threat model.

Ring VRFs avoid deanonymization by a group master and impose a coherent
structure upon the authorization conditions, with all signers' keys
...
\item[]
 ...
 Next the challenger fixes $j=0$ or $j=1$ and gives
  $\adv$ a signature $\sigma = \Sign(\sk_{i_j},\openring,\msg,\aux)$.
...
\item $\rVRF.\rSign : (\sk,\openring,\msg,\aux) \mapsto \sigma$ takes
 ...
 \begin{enumerate}
 \item computes the ring membership proof $\pi$ and associated \openpk,
  % $$ \pi = \NIZK.\Prove \Setst{ (\compk,\comring) }{ \OpenKey(\compk,\openpk) = \OpenRing(\comring,\opring) } \mathcomma $$
 \item computes the VRF-AC-KC signature
  $\sigma = \PedVRF.\Sign(\sk,\openpk,\msg,\aux \doubleplus \compk \doubleplus \pi)$, and finally
 \item returns the ring VRF signature $\rho = (\compk,\pi,\sigma)$.
 \end{enumerate}
\item $\rVRF.\rVerify$ takes $(\comring,\msg,\aux,\rho)$,
parses $\rho$ as $(\compk,\pi,\sigma,)$,  and then
returns $\PedVRF.\Verify(\compk,\msg,\aux \doubleplus \compk \doubleplus \pi,\sigma)$
 iff $\NIZK.\Verify(\pi,\compk,\comring)$ succeeds. 
\end{itemize}

In this, we tie $\sigma$ to $\pi$ by expanding $\sigma$'s auxiliary data with $\pi$.
...
\item {\em Ring } - 
 ...
 even given $\ctx_0 = \{ \pk_1,\ldots,\pk_i \}$ and
  chosen-message queries to $\rVRF.\Sign(\sk_i,\openring)$,
  except with odds negligible in $\lambda$.
\end{itemize}
