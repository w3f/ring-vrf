...
\item[]
 ...
 $n$ valid ring VRF signature $\sigma$, each with distinct outputs,
 for $\ctx$ on a message \msg and associated data \aux
 provided that
 $\adv$ invoked $\ora{Sign}$ on $\msg,\aux$ and $i$ with $\pk_i \in \ctx$
  strictly fewer than $n - |\ctx \setminus \ctx_0|$ times.
\end{itemize}
\end{definition}
...
\subsection{rVRF-AD instantiation}
