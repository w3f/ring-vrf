...
We can apply the results of the previous subsections to construct a ringVRF using $\SpecialG$ that allows a\eprint{n incredibly }{ }fast amortized ring VRF prover. \PedVRF has a sigma protocol which proves the relation $\reltwo=\mathcal{R}_{eval}$ and we can then use SpecialG for relation $\relone$ similar to $\Rring$.
