...
to generate a ring VRF signature for the same ring and for a different input, we need to run $\SpecialG.\Reprove$ and $\NIZK_{\mathcal{R}_{eval}}.\Prove$. $\NIZK_{\mathcal{R}_{eval}}.\Prove$ requires only three scalar multiplications and $\SpecialG.\Reprove$ requires five: two in $G_2$ and 3 in $G_1$.
