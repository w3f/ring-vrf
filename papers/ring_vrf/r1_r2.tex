...
We can apply the results of the previous subsections to construct a ringVRF using $\SpecialG$ that allows a\eprint{n incredibly }{ }fast amortized ring VRF prover. \PedVRF has a sigma protocol which proves the relation $\reltwo=\mathcal{R}_{eval}$ and we can then use SpecialG for relation $\relone=\Rring$.
...
$$ \relone = \Setst{ \sk_0 + 2^{\lambda} \sk_1, \comring }{
 \eprint{ \exists d,\openring \textrm{\ s.t.\ } }{}
 % 0 < \sk_0,\sk_1 < 2^{\lambda} \textrm{\ and\ } 
 \genfrac{}{}{0pt}{}{ \eprint{\rVRF.}{}\OpenRing(\comring,\openring) }{ \,\, = \sk_0 \genJ_0 + \sk_1 \genJ_1 + d \genJ_2 }
} \mathperiod $$

If we have a proof for $\relone$ for our $\pk$ in a ring $\comring$, to generate a RingVRF proof for the same ring, we need to run $\SpecialG.\Reprove$ and $\PedVRF.\Sign$. $\PedVRF.\Sign$ requires two scalar multiplications on $\ecE_1$
